...
- (void)addHeaderView{
    ...
    if ([self.jiemuImages containsString:@"http"]) {
        [img sd_setImageWithURL:[NSURL URLWithString:self.jiemuImages] placeholderImage:[UIImage imageNamed:@"tingwen_bg_square"]];
    }else{
...
- (void)backAction:(UIButton *)sender {
    if (self.listVC) {
        ...
        BOOL isExist = NO;
        NSMutableArray *marr = [[NSMutableArray alloc] initWithArray:self.navigationController.childViewControllers];
        for (UIViewController *vc in marr) {
            if ([vc isKindOfClass:[guanggaoVC class]]) {
                isExist = YES;
                break;
            }
        }
