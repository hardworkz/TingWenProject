...
- (void)setPost_id:(NSString *)post_id
{
    ...
    if ([CommonCode readFromUserD:pushNews]) {
        [self loadData];
        if (self.reloadNewsTableView) {
...
- (void)loadData
{
    ...
    [NetWorkTool getPostDetailWithaccessToken:AvatarAccessToken post_id:self.post_id sccess:^(NSDictionary *responseObject) {
        ...
        if ([responseObject[status] intValue] == 1){
            ...
            //新闻详情数据补全
            _postDetailModel.is_collection = responseObject[results][@"is_collection"];
            _postDetailModel.user_zan = responseObject[results][@"user_zan"];
            //设置详情头部状态控件数据
    ...
    [NetWorkTool getNewSlideListWithaccessToken:ExdangqianUser?[DSE encryptUseDES:ExdangqianUser]:@"" sccess:^(NSDictionary *responseObject) {
        if ([responseObject[results] isKindOfClass:[NSArray class]]){
            ...
            for (int i = 0; i<self.slideADResult.count; i++) {
                NSDictionary *data = self.slideADResult[i];
                if ([data[@"cate"] intValue] != 1) {
                    [tempArray addObject:data];
                }
            }
            self.slideADResult = tempArray;
        }else{
            [self.slideADResult removeAllObjects];
        }
        [self.tableView reloadData];
    } failure:^(NSError *error) {
        [self.slideADResult removeAllObjects];
...
- (void)setupTableViewHeaderData
{
    ...
    //设置主播头像
    if ([self.postDetailModel.act.images containsString:@"http"]) {
        [self.zhuboImg sd_setImageWithURL:[NSURL URLWithString:self.postDetailModel.act.images] placeholderImage:[UIImage imageNamed:@"thumbnailsdefault"]];
        [self.zhuboImgNav sd_setImageWithURL:[NSURL URLWithString:self.postDetailModel.act.images] placeholderImage:[UIImage imageNamed:@"thumbnailsdefault"]];
    }else{
...
//        weakSelf.yinpinzongTime.text = [self convertStringWithTime:totalDuration];
    };
...
//        [_tableView addTapGesWithTarget:self action:@selector(tableViewTap)];
        _tableView.tableFooterView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 109.0 / 667 * IPHONE_H)];
    }
    return _tableView;
}
//- (void)tableViewTap
...
- (UISlider *)sliderProgress
{
    if (!_sliderProgress)
    {
        ...
        [_sliderProgress addGestureRecognizer:tap];
    }
...
- (NSMutableArray *)slideADResult
{
    if (!_slideADResult)
    {
        _slideADResult = [NSMutableArray array];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([tableView isEqual:self.tableView]) {
        if (indexPath.row == 0) {
            ...
            ShufflingTableViewCell *cell = [ShufflingTableViewCell cellWithTableView:tableView];
            cell.slideADResult = self.slideADResult;
            return cell;
        }else{
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([tableView isEqual:self.tableView]) {
        if (indexPath.row == 0) {
            ...
            return IS_IPHONEX?162.0:162.0 / 667 * SCREEN_HEIGHT;
        }
//        else if (indexPath.row == 1) {
//            return 72.0;
//        }else if (indexPath.row == 2) {
...
//        RTLog(@"contentOffsetY:%f-----lastContentOffsetY:%f",scrollView.contentOffset.y,_lastContenOffsetY);
...
- (void)pinglunAction
{
    if ([[CommonCode readFromUserD:@"isLogin"] boolValue] == YES){
        ...
        if ([[CommonCode readFromUserD:@"isIAP"] boolValue] == YES) {
            ...
            [qingshuruyonghuming addAction:[UIAlertAction actionWithTitle:@"前往登录" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                LoginVC *loginFriVC = [LoginVC new];
                LoginNavC *loginNavC = [[LoginNavC alloc]initWithRootViewController:loginFriVC];
                [loginNavC.navigationBar setBackgroundColor:[UIColor whiteColor]];
                loginNavC.navigationBar.tintColor = [UIColor blackColor];
...
- (void)achorVCDidClickedListPlayBtn
{
    DefineWeakSelf
    [ZRT_PlayerManager manager].playDidEnd = ^(NSInteger currentSongIndex) {
        RTLog(@"回调到播放器-----------------------------------achorVCDidClickedListPlayBtn");
        //设置详情模型
        weakSelf.postDetailModel = [newsDetailModel new];
        //保存当前播放新闻的ID
        [weakSelf saveCurrentPlayNewsID];
...
//        AVPlayerItem *item = [ZRT_PlayerManager manager].playerItem;
//        CGFloat fps = [[[item.asset tracksWithMediaType:AVMediaTypeVideo] objectAtIndex:0] nominalFrameRate];
//        CMTime time = CMTimeMakeWithSeconds([ZRT_PlayerManager manager].duration * slider.value, fps);
//        [[ZRT_PlayerManager manager].player seekToTime:CMTimeMake(self.sliderProgress.value, [ZRT_PlayerManager manager].playRate) toleranceBefore:kCMTimeZero toleranceAfter:kCMTimeZero];
        [[UIDevice currentDevice] setProximityMonitoringEnabled:[[NSUserDefaults standardUserDefaults] boolForKey:@"shoushi"]];
    }else{
...
//    [self.sliderProgress setValue:value animated:YES];
...
{
    if (IPHONE_H - (IS_IPHONEX ? 50 + 34:50) != dibuView.y) {
        [UIView animateWithDuration:0.5 animations:^{
            dibuView.y = IPHONE_H - (IS_IPHONEX ? 50 + 34:50);
        }];
    }
