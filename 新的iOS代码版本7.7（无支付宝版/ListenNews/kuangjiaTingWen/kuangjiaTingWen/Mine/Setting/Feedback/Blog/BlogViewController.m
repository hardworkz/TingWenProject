...
- (NSString *)PostImagesToServer:(NSString *)strUrl
    ...
    [manager POST:strUrl parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        NSArray *keys = [dicImages allKeys];
        // formData: 专门用于拼接需要上传的数据,在此位置生成一个要上传的数据体
        // 这里的_photoArr是你存放图片的数组
        for (int i = 0; i < keys.count; i++) {
            UIImage *image = [dicImages objectForKey:[keys objectAtIndex:i]];
             ...
             *该方法的参数
             ...
             4. mimeType：上传的文件的类型
             */
            [formData appendPartWithFileData:imageData name:[NSString  stringWithFormat:@"images%d",i] fileName:fileName mimeType:@"multipart/form-data"]; //
        }
        ...
        if (self.isFeedbackVC) {
            [self addNewBlogWithContent:contentQA photos:responseObject[results] location:location];
        }
        else{
            [self addLinsteningCircleWithContent:contentQA timages:responseObject[results] mp3_url:self.mp3_url playtime:self.playtime];
        }

    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        NSLog(@"xxx上传失败xxx %@", error);
        [SVProgressHUD dismiss];
        [[XWAlerLoginView alertWithTitle:@"上传失败了，请重试~"] show];
    }];
...
//        NSLog(@"err code : %@ %@", [err localizedDescription],err);
//    }
