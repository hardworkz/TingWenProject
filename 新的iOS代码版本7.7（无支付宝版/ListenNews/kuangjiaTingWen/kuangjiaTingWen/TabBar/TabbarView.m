...
- (void)setItems:(NSArray *)items{
    ...
    [items enumerateObjectsUsingBlock:^(UITabBarItem * _Nonnull tabBarItem, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        //水波纹扩散动画
        if (idx == 1) {
            if (![[CommonCode readFromUserD:@"animationTip"] boolValue]) {
                RippleAnimationView *viewA = [[RippleAnimationView alloc] initWithFrame:CGRectMake(0, 0, 10, 10) animationType:AnimationTypeWithBackground];
                viewA.multiple = 3.0;
                viewA.userInteractionEnabled = NO;
                viewA.center = _btnImgView.center;
                [tabBarBtn insertSubview:viewA atIndex:0];
...
- (void)tabBarBtnClick:(UIButton *)btn{
    ...
    for (UIView *subView in self.seletBtn.subviews) {
        if ([subView isKindOfClass:[UIImageView class]]) {
            ((UIImageView *)subView).highlighted = NO;
        }
        if ([subView isKindOfClass:[UILabel class]]) {
            ((UILabel *)subView).textColor = TITLE_COLOR_HEX;
        }
    }
    for (UIView *subView in btn.subviews) {
        ...
        if ([subView isKindOfClass:[UILabel class]]) {
            ((UILabel *)subView).textColor = nMainColor;
        }
        if ([subView isKindOfClass:[RippleAnimationView class]]) {
            [UIView animateWithDuration:0.5 animations:^{
                subView.alpha = 0;
            } completion:^(BOOL finished) {
    ...
    if (_currentIdx == 1) {
        [CommonCode writeToUserD:@(YES) andKey:@"animationTip"];
    }
