...
- (void)viewDidLoad{
    ...
    URLListController  *dingyue = [[URLListController alloc]init];
    ...
    [self tabBarChildViewController:dingyue tabBarTitle:@"帮你读" norImage:[UIImage imageNamed:@"home_tab_subscribe"] selImage:[UIImage imageNamed:@"home_tab_subscribe_select"]];
    ...
    [NetWorkTool getpostinfoWithpost_id:[CommonCode readFromUserD:pushNews] andpage:nil andlimit:nil sccess:^(NSDictionary *responseObject) {
        if ([responseObject[status] integerValue] == 1) {
            ...
            for (int i = 0;i<array.count;i++) {
                ...
                if (dict[@"id"] == responseObject[results][@"post_id"]) {
                    isExist = YES;
                    index = i;
                    break;
                }
            ...
            [weakSelf presentPushNewsWithArray:array index:index];
        }
        else{
...
/**
 通知消息点击跳转新闻详情界面播放
 */
- (void)presentPushNewsWithArray:(NSMutableArray *)array index:(NSInteger)index
{
    //当前播放新闻ID
    [CommonCode writeToUserD:self.pushNewsInfo[@"post_id"] andKey:dangqianbofangxinwenID];
    //保存已听过新闻的ID数据
    [[NewPlayVC shareInstance].listenedNewsIDArray addObject:self.pushNewsInfo[@"post_id"]];
    NSSet *set = [NSSet setWithArray:[NewPlayVC shareInstance].listenedNewsIDArray];
    [NewPlayVC shareInstance].listenedNewsIDArray = [NSMutableArray arrayWithArray:[set allObjects]];
    ...
    [[NewPlayVC shareInstance] playFromIndex:index];
    //设置新闻ID
