...
//        [self.splash loadAdAndShowInWindow:self.window withBottomView:_bottomView];
    ...
    NSString *initString = [[NSString alloc] initWithFormat:@"appid=%@", @"5afd1ec6"];
...
- (void)applicationWillEnterForeground:(UIApplication *)application
{
    ...
    RTLog(@"%@---%@",pasteboard.URL,pasteboard.string);
    if (pasteboard.URL && [pasteboard.URL.description hasPrefix:@"http"]) {
        ...
        [CommonCode writeToUserD:titleResult.length >= 3? [titleResult substringWithRange:NSMakeRange(1, titleResult.length - 2)]:titleResult andKey:@"copyTitle"];
    }
